...
exports.CreateDebts = async (req, res) => {
  ...
  const { firstname, secondname, email, contact, money, enddate, description } =
    req.body;
...
exports.GetDebtHistory = async (req, res) => {
  try {
    ...
    // response.all.length!==0?res.json(response):res.send("no data found")
    res.json(response);
  } catch (err) {
    console.error(err);
...
exports.updateDebt = async (req, res) => {
  ...
  try {
    ...
    if (prevDebt.money >= parseInt(money) && prevDebt.money !== 0) {
      ...
      const updatedDebt = await Debt.findOneAndUpdate({ _id: id }, updates, {
        new: true,
        runValidators: true,
      });
